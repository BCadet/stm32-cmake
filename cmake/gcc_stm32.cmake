...
SET(CMAKE_MODULE_PATH ${STM32_CMAKE_DIR} ${STM32_CMAKE_DIR}/../nanopb/extra ${CMAKE_MODULE_PATH})
...
FUNCTION(STM32_PARSE_LINKER_SCRIPT TARGET)
    IF(NOT STM32_CHIP_TYPE)
        IF(NOT STM32_CHIP)
            MESSAGE(WARNING "Neither STM32_CHIP_TYPE nor STM32_CHIP selected, you'll have to use STM32_SET_CHIP_DEFINITIONS directly")
        ELSE()
    ...
    STM32_SET_CHIP_DEFINITIONS(${TARGET} ${STM32_CHIP_TYPE})
    IF(NOT STM32_LINKER_SCRIPT)
        MESSAGE(FATAL_ERROR "You didn't provide a linker script. Prease set STM32_LINKER_SCRIPT or use STM32_SET_TARGET_PROPERTIES() instead.")
    ELSE()
        file(STRINGS ${STM32_LINKER_SCRIPT} LS REGEX "ORIGIN = 0x[0-9A-Fa-f]*,")
        list(GET LS 1 FLASH_STR)
        list(GET LS 0 RAM_STR)
        string(REGEX MATCH "0x[0-9A-Fa-f]*" FLASH_ORIGIN ${FLASH_STR})
        string(REGEX MATCH "0x[0-9A-Fa-f]*" RAM_ORIGIN ${RAM_STR})
        string(REGEX MATCH "[0-9A-Fa-f]*K" FLASH_SIZE ${FLASH_STR})
        string(REGEX MATCH "[0-9A-Fa-f]*K" RAM_SIZE ${RAM_STR})
        SET(STM32_FLASH_ORIGIN ${FLASH_ORIGIN} CACHE INTERNAL "stm32 flash origin")
        SET(STM32_FLASH_SIZE ${FLASH_SIZE} CACHE INTERNAL "stm32 flash size")
        SET(STM32_RAM_ORIGIN ${RAM_ORIGIN} CACHE INTERNAL "stm32 ram origin")
        SET(STM32_RAM_SIZE ${RAM_SIZE} CACHE INTERNAL "stm32 ram size")
        STM32_SET_FLASH_PARAMS(${TARGET} ${STM32_FLASH_SIZE} ${STM32_RAM_SIZE})
        MESSAGE(STATUS "${STM32_CHIP} has ${STM32_FLASH_SIZE}iB of flash memory and ${STM32_RAM_SIZE}iB of RAM according to the linker script.")
    ENDIF()
ENDFUNCTION()